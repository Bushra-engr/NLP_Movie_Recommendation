...
# CONFIG
# =============================
# Docker environment variable se API_BASE uthayega, default localhost rahega
...
if not api_key:
    st.sidebar.warning("⚠️ TMDB_API_KEY missing in environment.")
...
if st.session_state.view == "home":
    typed = st.text_input("Search movie title...", placeholder="e.g. Inception, Avatar")
    st.divider()
        ...
        if len(typed.strip()) < 2:
            ...
            if err: st.error(f"Search failed: {err}")
                ...
                if suggestions:
                    ...
                    selected = st.selectbox("Quick Select", labels, index=0)
...
# =============================
    ...
    if err or not data:
        st.error(f"Could not load movie data: {err}")
        st.stop()
    ...
    with right:
        ...
        st.info(f"📅 Release: {data.get('release_date', 'N/A')}")
